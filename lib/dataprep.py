...
def leave_last_out(data, userid='userid', timeid='timestamp'):
    ...
    remaining = remaining[remaining[userid].isin(set(remaining[userid].unique()) & set(holdout[userid].unique()))]
    holdout = holdout[holdout[userid].isin(set(remaining[userid].unique()) & set(holdout[userid].unique()))]
...
def split_by_time(data: pd.DataFrame,
                            split_time,
                            userid='userid',
                            timeid='timestamp'):
...
def tvt_data_split(data, val_ts, test_ts, ts = 'timestamp'):
    return data[data[ts] < val_ts], data[(data[ts] >= val_ts) & (data[ts] < test_ts)], data[data[ts] >= test_ts]


def pad_sequences(sequences, max_len=None):
    if max_len is None:
        max_len = sequences.map(len).max()
    
    n_items = sequences.index.max() + 1
    padded = np.zeros((n_items, max_len), dtype=int)
    for item_idx, seq in sequences.items():
        trunc = seq[:max_len]
        padded[item_idx, :len(trunc)] = trunc

    return padded, max_len


def encode_varlen(feat, sep=','):
    clear = feat \
        .fillna('AA_Unknown') \
        .str.split(sep) \
        .explode() \
    ...
    codes = clear.astype('category').cat.codes
    index = {i: v for i, v in enumerate(clear.astype('category').cat.categories)}
    grouped = codes.groupby(level=0).apply(list)

    return grouped, index


def encode_entity_features(entity_metadata, data_index, entity):
    
    entity_features = {}
    field = data_index[entity].name  

    for feat in entity_metadata.columns.drop(field):
        encoded, index = encode_varlen(entity_metadata.set_index(field)[feat])
        padded, _ = pad_sequences(encoded)
        entity_features[feat] = {'index': index, 'padded': padded}
